...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			$this->logger->critical( esc_html__( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'meddox' ) );
		...
		if ( ! $status ) {
			$this->logger->error( esc_html__( 'Could not open the XML file for parsing!', 'meddox' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->error( esc_html__( 'Content import start error: ', 'meddox' ) . $result->get_error_message() );
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'meddox' ),
							$this->version,
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
			...
			$this->logger->info( esc_html__( 'New AJAX call!', 'meddox' ) );

...
class PHPConsoleHandlerTest extends TestCase
{
    ...
    public function testError($galleryPartialsTraceIgnore = null)
    {
        $code = E_USER_NOTICE;
        $message = 'message';
        $file = __FILE__;
        ...
        $this->errorDispatcher->expects($this->once())->method('dispatchError')->with(
            ...
            $this->equalTo($file),
            $this->equalTo($line),
            $galleryPartialsTraceIgnore ?: $this->equalTo($this->getHandlerDefaultOption('galleryPartialsTraceIgnore'))
        );
        $errorHandler = ErrorHandler::register($this->initLogger($galleryPartialsTraceIgnore ? array('galleryPartialsTraceIgnore' => $galleryPartialsTraceIgnore) : array()), false);
    ...
    public function testOptionGalleryPartialsTraceIgnore()
    {

<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		if ( 'valid' != get_option( meddox()->get_slug().'_purchase_code_status', false ) && !$dev_mode ) :
			
			echo '<div class="error"><p>' .
					sprintf( wp_kses_post( esc_html__( 'The %s theme needs to be registered. %sRegister Now%s', 'meddox' ) ), meddox()->get_name(), '<a href="' . admin_url( 'admin.php?page=pxlart') . '">' , '</a>' ) . '</p></div>';
		elseif( !empty($plugin_requires) && sizeof($plugin_requires) >= 1 ):

			echo '<div class="error">';
			echo sprintf( wp_kses_post( esc_html__( 'Make sure to activate required plugins prior to import a demo to %s. %sActive Now%s', 'meddox' ) ), meddox()->get_name(), '<a class="nt-atpli" href="' . admin_url( 'admin.php?page=pxlart-plugins') . '">' , '</a>' ) . '</p>';
		...
		<header class="pxl-dsb-header">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Import a Demo', 'meddox' ); ?></h4>
				<p><?php esc_html_e( 'Choose a pre-built website for starting a quick design process.', 'meddox' ) ?></p>
			</div>
			<div class="pxl-msg pxl-dsb-notice">
				<p><span><?php esc_html_e( 'Note:', 'meddox' ); ?></span> <?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'meddox' ) ?></p>
			</div>
		</header>
							...
							<figure>
								...
								<div class="pxl-dsb-overlay">
									<a href="#" id="import-id" data-import-id="<?php echo esc_attr( $i ); ?>" data-demo-id="<?php echo esc_attr( $id ); ?>" class="pxl-btn pxl-popup-import">
										<span><?php esc_html_e( 'Import Demo', 'meddox' ); ?></span>
									</a>
									<a target="_blank" href="<?php echo esc_url( $demo['preview'] ); ?>" class="pxl-btn pxl-preview-btn">
										<span><?php esc_html_e( 'Preview', 'meddox' ); ?></span>
		...
		<script type="text/template" id="tmpl-demo-import-modules">
			<div id="pxl-progress-popup" class="pxl-imp-popup-wrap is-active">
				<div class="pxl-imp-progress">
					<h6><?php esc_html_e( 'Importing...', 'meddox' ); ?></h6>
					<div id="pxl-progress" class="importing"><?php esc_html_e( 'Working', 'meddox' )?> <span>.</span><span>.</span><span>.</span></div>
					<div class="pxl-progressbar">
						<div class="pxl-progressbar-inner" style="width: 0%">
							<span id="pxl-loader" class="pxl-progressbar-percentage"><?php esc_html_e( '0%', 'meddox' ); ?></span>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="pxl-popup" class="pxl-imp-popup-wrap is-active">
				<div class="pxl-imp-popup-inner">
					...
					<div class="pxl-imp-popup-content">
						<h4 style="text-align:center; margin-bottom: 30px;"><?php esc_html_e( 'Select all or a few', 'meddox' ); ?></h4>
						<div class="pxl-row">
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Media Attachments', 'meddox' ); ?></span>
								</span>
							</div>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Content', 'meddox' ); ?></span>
								</span>
							</div>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Theme Options', 'meddox' ) ?></span>
								</span>
							</div>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Widgets', 'meddox' ); ?></span>
								</span>
							</div>
							...
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Revslider', 'meddox' ); ?></span>
								</span>
							</div>
 							<?php endif; ?>
							<div class="pxl-col pxl-col-6">
								<span class="pxl-imp-opt">
									...
									<span><?php esc_html_e( 'Settings', 'meddox' ) ?></span>
								</span>
							</div>
						</div>
							...
							<div class="pxl-col pxl-col-12">
								<div class="pxl-imp-crop">
									<span class="pxl-imp-opt-crop" style="margin-bottom: 0; padding-left: 15px;">
										...
										<span><?php esc_html_e( 'Crop Image after import finish?', 'meddox' ); ?></span>
									</span>
								</div>

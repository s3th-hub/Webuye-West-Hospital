...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'meddox');
...
$merlin_setup = get_option( 'merlin_' . meddox()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'meddox');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'meddox');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'meddox');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'meddox' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'meddox' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'meddox' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Meddox_Admin_Plugins;

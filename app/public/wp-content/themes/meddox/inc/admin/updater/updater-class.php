<?php
/**
 * The Meddox_Updater initiate the theme engine
 */

if( !defined( 'ABSPATH' ) ) 
	exit; // Exit if accessed directly
	
class Meddox_Updater {
	...
	function update_nag() {
		...
		if ( version_compare( $this->version, $api_response->new_version, '<' ) ) {
			...
			printf(
				wp_kses_post( esc_html__( '<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'meddox' ) ),
				$theme->get( 'Name' ),

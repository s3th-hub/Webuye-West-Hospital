...
class CASE_Breadcrumb
{
    ...
    function __construct( $args = array() )
    {
        ...
        if($page_on_front) {
            ...
            $home_label = esc_html__( 'Home', 'meddox' );
        }
        $args = wp_parse_args( $args, array(
            'home_label'            => $home_label,
            '404_label'             => esc_html__( 'Not Found', 'meddox' ),
            'search_results_label'  => esc_html__( 'Search Results', 'meddox' ),
    ...
    {
        $queried_objepxl = $GLOBALS['wp_query']->get_queried_object();
        $this->add_entry(
            esc_html__( 'Tag:', 'meddox' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_objepxl->term_id
        );
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'meddox' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'meddox_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'meddox_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
         ...
         */
        $entries = apply_filters( 'meddox_breadcrumb_single', $entries, $post );
